    ...
    <script>
      (function() {
        ...
        if (typeof ePub === 'undefined') {
          ...
          if (holder) {
            holder.innerHTML = '<p class="text-center text-white">EPUB viewer unavailable.</p>';
          }
          return;
        }
        ...
        fetch("<?php echo h($fileUrl); ?>", { credentials: 'same-origin' })
          .then(function(res) {
            if (!res.ok) throw new Error('Failed to load EPUB file');
            return res.arrayBuffer();
          })
          .then(function(buffer) {
            const book = ePub({ encoding: 'binary', replacements: 'none' });
            return book.open(buffer, "binary").then(function() {
          ...
          .catch(function(err) {
            ...
            if (holder) {
              holder.innerHTML = '<p class="text-center text-white">Failed to display EPUB.</p>';
            }
          });
      })();
    </script>
